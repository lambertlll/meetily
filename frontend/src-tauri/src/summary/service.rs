...
impl SummaryService {
    ...
    pub async fn process_transcript_background<R: tauri::Runtime>(
        ...
        // Read manual notes for this meeting
        let manual_notes = match ManualNotesRepository::get_notes(&pool, &meeting_id).await {
            Ok(notes) => {
                if let Some(ref n) = notes {
                    info!("Found manual notes for meeting {} ({} chars)", meeting_id, n.len());
                }
                notes
            }
            Err(e) => {
                warn!("Failed to read manual notes for meeting {}: {}. Proceeding without.", meeting_id, e);
                None
            }
        };
