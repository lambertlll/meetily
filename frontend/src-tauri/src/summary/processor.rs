...
pub async fn generate_meeting_summary(
        ...
        resolve_cached_english(cached_english, summary_language)
    {
        ...
        // Inject manual notes if provided
        if let Some(notes) = manual_notes {
            if !notes.trim().is_empty() {
                info!("Injecting manual notes ({} chars) into final prompt", notes.len());
                final_user_prompt.push_str("\n\n<manual_notes>\n");
                final_user_prompt.push_str(notes);
                final_user_prompt.push_str("\n</manual_notes>\n\n");
                final_user_prompt.push_str(
                    "**IMPORTANT - Manual Notes Integration Rules:**\n\
                     1. The <manual_notes> above are the user's key observations captured during the meeting.\n\
                     2. Use them as the PRIMARY structural backbone for the final report.\n\
                     3. Every point in manual notes MUST appear prominently in the output.\n\
                     4. Use transcript details to expand, clarify, and add context to manual note points.\n\
                     5. If manual notes mention action items or decisions, prioritize them.\n\
                     6. Maintain the logical order from the manual notes where possible.\n\
                     7. If transcript contains important info NOT in manual notes, add it as supplementary.\n"
                );
            }
        }
